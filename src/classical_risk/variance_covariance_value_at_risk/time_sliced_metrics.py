"""
Time-sliced metrics computation for VaR backtesting.

Computes metrics for specific time periods (e.g., by year) to analyze
temporal patterns in VaR performance.
"""
import pandas as pd
import numpy as np
from typing import Dict, List, Optional
from multiprocessing import Pool, cpu_count
from functools import partial
from .backtesting import (
    detect_var_violations,
    compute_hit_rate,
    compute_violation_ratio
)


def _process_time_slice(
    time_period_data: tuple,
    aligned_returns: pd.Series,
    aligned_var: pd.Series,
    time_groups: pd.Series,
    confidence_level: float,
    slice_format: callable
) -> Optional[Dict]:
    """
    Process a single time slice (worker function for parallelization).
    
    Args:
        time_period_data: Tuple of (time_period, expected_violation_rate)
        aligned_returns: Aligned returns series
        aligned_var: Aligned VaR series
        time_groups: Series of time group labels
        confidence_level: Confidence level used for VaR
        slice_format: Function to format time period string
        
    Returns:
        Dictionary with metrics for the time slice, or None if empty
    """
    time_period, expected_violation_rate = time_period_data
    
    # Get data for this time period
    mask = time_groups == time_period
    period_returns = aligned_returns[mask]
    period_var = aligned_var[mask]
    
    if len(period_returns) == 0:
        return None
    
    # Compute violations
    violations = detect_var_violations(period_returns, period_var, confidence_level)
    
    # Compute metrics
    hit_rate = compute_hit_rate(violations)
    num_violations = violations.sum()
    expected_violations = len(violations) * expected_violation_rate
    violation_ratio_val = compute_violation_ratio(violations, expected_violation_rate)
    
    # Get date range
    period_dates = period_returns.index
    start_date = period_dates.min().strftime('%Y-%m-%d')
    end_date = period_dates.max().strftime('%Y-%m-%d')
    
    time_slice = {
        'slice': slice_format(time_period),
        'start_date': start_date,
        'end_date': end_date,
        'hit_rate': float(hit_rate),
        'num_violations': int(num_violations),
        'expected_violations': float(expected_violations),
        'violation_ratio': float(violation_ratio_val)
    }
    
    return time_slice


def compute_time_sliced_metrics(
    returns: pd.Series,
    var_series: pd.Series,
    confidence_level: float = 0.95,
    slice_by: str = 'year',
    n_jobs: int = 1,
    enable_parallel: bool = True
) -> List[Dict]:
    """
    Compute metrics for time slices (e.g., by year).
    
    Supports parallel processing for better performance with many time slices.
    
    Args:
        returns: Series of actual returns with dates as index
        var_series: Series of VaR values with dates as index
        confidence_level: Confidence level used for VaR
        slice_by: How to slice time ('year', 'quarter', 'month')
        n_jobs: Number of parallel workers (1 for sequential)
        enable_parallel: Whether to enable parallelization
        
    Returns:
        List of dictionaries with metrics for each time slice
    """
    # Align returns and VaR
    common_dates = returns.index.intersection(var_series.index)
    if len(common_dates) == 0:
        return []
    
    aligned_returns = returns.loc[common_dates]
    aligned_var = var_series.loc[common_dates]
    
    # Create time slices
    if slice_by == 'year':
        time_groups = aligned_returns.index.year
        slice_format = lambda year: str(year)
    elif slice_by == 'quarter':
        time_groups = aligned_returns.index.to_period('Q')
        slice_format = lambda period: str(period)
    elif slice_by == 'month':
        time_groups = aligned_returns.index.to_period('M')
        slice_format = lambda period: str(period)
    else:
        raise ValueError(f"Unknown slice_by: {slice_by}. Use 'year', 'quarter', or 'month'")
    
    expected_violation_rate = 1 - confidence_level
    unique_periods = sorted(time_groups.unique())
    
    # Prepare data for parallel processing
    time_period_data_list = [(period, expected_violation_rate) for period in unique_periods]
    
    if enable_parallel and n_jobs > 1 and len(unique_periods) > 1:
        # Parallel processing
        try:
            worker_func = partial(
                _process_time_slice,
                aligned_returns=aligned_returns,
                aligned_var=aligned_var,
                time_groups=time_groups,
                confidence_level=confidence_level,
                slice_format=slice_format
            )
            
            with Pool(processes=min(n_jobs, len(unique_periods))) as pool:
                time_slices = pool.map(worker_func, time_period_data_list)
            
            # Filter out None results
            time_slices = [ts for ts in time_slices if ts is not None]
            
        except Exception:
            # Fall back to sequential processing
            time_slices = []
            for time_period_data in time_period_data_list:
                result = _process_time_slice(
                    time_period_data,
                    aligned_returns,
                    aligned_var,
                    time_groups,
                    confidence_level,
                    slice_format
                )
                if result is not None:
                    time_slices.append(result)
    else:
        # Sequential processing
        time_slices = []
        for time_period_data in time_period_data_list:
            result = _process_time_slice(
                time_period_data,
                aligned_returns,
                aligned_var,
                time_groups,
                confidence_level,
                slice_format
            )
            if result is not None:
                time_slices.append(result)
    
    return time_slices

