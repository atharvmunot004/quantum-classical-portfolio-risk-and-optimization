...
- Data loading
- Returns computation
- Rolling VaR and CVaR calculation via GARCH volatility forecasting
- Backtesting
- Metrics computation
- Report generation
"""
...
# Suppress GARCH convergence warnings (these are handled by fallback mechanism)
        ...
        var_eval = {
            ...
            'global_metrics': {
                ...
                'runtime': {
                    ...
                    'garch_fitting_time_ms': result.get('garch_fitting_time_ms', np.nan),
                    'forecast_time_ms': result.get('forecast_time_ms', np.nan),
...
def _save_restructured_json(
    ...
    Args:
        ...
        garch_settings: GARCH model settings
    """
...
def _process_single_portfolio(
    ...
    results = []
    garch_runtimes = []  # Track GARCH fitting time per configuration
    forecast_runtimes = []  # Track forecast time per configuration
    ...
    for confidence_level in confidence_levels:
        for horizon in horizons:
            for window in estimation_windows:
                try:
                    # Track time for GARCH fitting and forecasting
                    garch_start_time = time.time()
                    
                    # Compute rolling VaR and CVaR from GARCH
                    rolling_var = compute_rolling_var_from_garch(
                        portfolio_returns,
                        window=window,
                    ...
                    rolling_cvar = compute_rolling_cvar_from_garch(
                        portfolio_returns,
                        window=window,
                        confidence_level=confidence_level,
                        horizon=horizon,
                    ...
                    garch_runtime = time.time() - garch_start_time
                    garch_runtimes.append(garch_runtime)
                    forecast_runtimes.append(garch_runtime)  # Forecast time is part of GARCH time
                    ...
                    result = {
                        ...
                        'var_runtime_ms': garch_runtime * 1000,
                        'garch_fitting_time_ms': garch_runtime * 1000,  # GARCH time includes fitting
                        'forecast_time_ms': garch_runtime * 1000,  # Forecast time is part of GARCH time
                        **accuracy_metrics,
                        **tail_metrics,
                        **cvar_tail_metrics,
                        **structure_metrics,
    ...
    garch_runtime_total = sum(garch_runtimes) if len(garch_runtimes) > 0 else 0.0
    
    return results, garch_runtime_total, aligned_data
...
def evaluate_garch_var_cvar(
    ...
    """
    Main function to evaluate VaR and CVaR using GARCH(1,1) volatility forecasting for multiple portfolios.
    ...
    print("=" * 80)
    print("GARCH(1,1) VOLATILITY FORECASTING FOR VAR/CVAR EVALUATION")
    print("=" * 80)
    print(f"\nLoading data...")
    ...
    horizons = garch_settings.get('horizons', [1, 10])
    ...
    use_mean_model = garch_settings.get('use_mean_model', False)
    ...
    print(f"  Fallback to long-run variance: {fallback_long_run_variance}")
    ...
    print(f"  Note: GARCH fitting is computationally intensive. This may take several minutes...")
    print(f"  Note: Convergence warnings may appear - these are non-critical and handled by fallback mechanism.")
        ...
        with Pool(processes=n_jobs) as pool:
            ...
            for results, runtime, aligned_data in results_iter:
                ...
                if should_print:
                    ...
                    # Flush output to ensure progress is visible even with warnings
    ...
    if len(runtimes) > 0:
        garch_runtime_total = sum(runtimes)
        avg_garch_runtime_per_portfolio = garch_runtime_total / num_portfolios if num_portfolios > 0 else 0
        print(f"  GARCH fitting/forecast runtime: {garch_runtime_total/60:.2f} minutes ({garch_runtime_total:.2f} seconds)")
        print(f"  Average GARCH runtime: {avg_garch_runtime_per_portfolio*1000:.2f} ms per portfolio")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Evaluate GARCH(1,1) Volatility Forecasting VaR/CVaR for portfolios'
    )
    parser.add_argument(
        '--config',
        type=str,
